...
def get_pixiewps_version():
    try:
        result = subprocess.run(['pixiewps', '-v'], capture_output=True, text=True, check=True)
        return result.stdout.strip()
    except Exception as e:
        print(f"[!] Error detecting Pixiewps version: {e}")
        return None
...
class PixiewpsData:
    def __init__(self):
        ...
        self.eh1_salt = ''
        self.eh2_salt = ''
        self.version = get_pixiewps_version()
    ...
    def got_all(self):
        ...
        if self.eh1_salt:
            cmd += f" --eh1-salt {self.eh1_salt}"
        if self.eh2_salt:
            cmd += f" --eh2-salt {self.eh2_salt}"
        if full_range:
            cmd += ' --force'
        return cmd
